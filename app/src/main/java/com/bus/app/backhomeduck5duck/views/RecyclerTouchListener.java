 ...
 */
package com.bus.app.backhomeduck5duck.views;
...
public class RecyclerTouchListener implements RecyclerView.OnItemTouchListener, OnActivityTouchListener {
    ...
    private OnSwipeOptionRevealListener onSwipeOptionRevealListener = null;
    // user choices
...
//        mSwipeListener = listener;
    ...
    public RecyclerView.AdapterDataObserver getDataObserver() {
        return new RecyclerView.AdapterDataObserver() {
            ...
            public void onItemRangeInserted(int positionStart, int itemCount) {
                ...
                Timber.d("item range inserted " + positionStart + " " + itemCount);
                if (bgVisible) {
                    boolean same = touchedPosition == bgVisiblePosition;
                    if (bgVisiblePosition >= positionStart) {
                        bgVisiblePosition += itemCount;
                    }
                    if (same) {
                        touchedPosition = bgVisiblePosition;
                    }
                }
            }
            ...
            public void onItemRangeRemoved(int positionStart, int itemCount) {
                ...
                Timber.d("item range removed " + positionStart + " " + itemCount);
                if (bgVisible) {
                    boolean same = touchedPosition == bgVisiblePosition;
                    if (bgVisiblePosition >= positionStart + itemCount) {
                        bgVisiblePosition -= itemCount;
                    }
                    if (same) {
                        touchedPosition = bgVisiblePosition;
            ...
            public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
                ...
                Timber.d("item range moved " +fromPosition + " " +toPosition + " " + itemCount);
                if (bgVisible) {
                    boolean same = touchedPosition == bgVisiblePosition;
    ...
    public Set<Integer> getIgnoredViewTypes() {
        return ignoredViewTypes;
    }
    ...
    public RecyclerTouchListener setOnSwipeOptionRevealListener(OnSwipeOptionRevealListener onSwipeOptionRevealListener) {
        this.onSwipeOptionRevealListener = onSwipeOptionRevealListener;
        return this;
    }
...
//                        bgView.getLayoutParams().height = fgView.getHeight();
            ...
            case MotionEvent.ACTION_UP: {
                ...
                // to it's original position
                    ...
                    bgVisibleView = null;
                    bgVisiblePosition = -1;
                    if (onSwipeOptionRevealListener != null) {
                        onSwipeOptionRevealListener.onSwipeOptionsHide(bgView, touchedPosition);
                    }
                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (!isFgSwiping && Math.abs(deltaX) > touchSlop && Math.abs(deltaY) < Math.abs(deltaX) / 2) {
                    handler.removeCallbacks(mLongPressed);
                    isFgSwiping = true;
                    if (!bgVisible && deltaX < 0 && this.onSwipeOptionRevealListener != null) {
                        reveal = true;
                    }
                ...
                if (swipeable && isFgSwiping && !unSwipeableRows.contains(touchedPosition)) {
                    ...
                    if (deltaX < touchSlop && !bgVisible) {
                        if (reveal) {
                            this.onSwipeOptionRevealListener.onSwipeOptionsReveal(bgView, touchedPosition);
                        }
...
//                        }
    ...
    public interface OnSwipeOptionRevealListener {
        void onSwipeOptionsReveal(View view, int position);
        void onSwipeOptionsHide(View view, int position);
    }
}